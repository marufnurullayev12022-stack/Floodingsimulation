...
export function enhancedShallowWaterStep(
  ...
  for (let i = 0; i < N; i++) {
    ...
    for (let k = 0; k < 4; k++) {
      ...
      // Limit outflow to not exceed cell capacity (CFL-like)
      const maxOutflow = Math.min(d * area * 0.5 / dt, outflowRate * dt) / dt;
...
export function depthToColorEnhanced(
  ...
  if (magnitude > 0.01) {
    const flow = Math.min(1, magnitude / 0.5);
    r = Math.round(r + 50 * flow);
    g = Math.round(g + 70 * flow);
    b = Math.round(b + 40 * flow);
  }
